...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      ...
      if (userErr || !userData?.user) {
        console.warn('No user available', userErr);
        setLoading(false);
        return;
      }
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('full_name, avatar_url, title, company_name')
        .eq('id', user.id)
        .single();
      ...
      if (error) {
        ...
        // populate from auth user fallback
        setProfile((p) => ({
          ...p,
          name: user.user_metadata?.full_name || user.email?.split('@')[0] || '',
          email: user.email || '',
          avatar: user.user_metadata?.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(user.email || '')}`,
        }));
        setFormData((f) => ({ ...f }));
        setLoading(false);
        return;
      ...
      setProfile((p) => ({
        ...
        avatar: data.avatar_url || user.user_metadata?.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(user.email || '')}`,
      }));
  ...
  const handleSave = () => {
    // Persist selected fields to Supabase `profiles` table.
    const save = async () => {
      const supabase = createSupabaseBrowserClient();
